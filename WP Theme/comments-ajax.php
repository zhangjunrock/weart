...
if ( $user->exists() ) {
	...
	if(dopt('tucao_page_id')){
		if( !current_user_can('level_10') && $_POST["comment_post_ID"] == dopt('tucao_page_id') && $_POST["comment_parent"]== 0 ) {
			err(__('这里只有博主才能吐槽，你声援就行了!'));
		}
	}
} else {
	if ( get_option('comment_registration') || 'private' == $status )
		err(__('对不起，请登陆后评论。')); // 將 wp_die 改為錯誤提示
	if(dopt('tucao_page_id')){
		if( $_POST["comment_post_ID"] == dopt('tucao_page_id') && $_POST["comment_parent"]== 0 ) err(__('这里只有博主才能吐槽，你声援就行了!'));
	}
}
$comment_type = '';
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('请填写你的大名和Email。') ); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
		err( __('请填写有效的Email地址。') ); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
	err( __('请填写评论内容！') ); // 將 wp_die 改為錯誤提示
