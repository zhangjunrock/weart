...
$zg_number_of_posts = 7; // 显示篇数，默认值是10。
...
function zg_recently_viewed() {
	...
	if (isset($_COOKIE["WP-LastViewedPosts"])) {
		...
		foreach ($zg_post_IDs as $value) {
			global $wpdb;
			$zg_get_title = $wpdb->get_results("SELECT post_title, post_date FROM $wpdb->posts WHERE ID = '$value+0' LIMIT 1");
			foreach($zg_get_title as $zg_title_out) {
				echo '<li><span class="r">' . date('m-d', strtotime($zg_title_out->post_date)) . '</span><a href="' . get_permalink($value+0) . '" title="' . $zg_title_out->post_title . '">' . $zg_title_out->post_title . '</a></li>';
			}
		}
	} else {
	}
